...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, num_frames: int, attn_mask: torch.Tensor = None, adapter: str = None):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, num_frames: int, attn_mask: torch.Tensor = None, adapter: str = None):
        super().__init__()
        self.width = width
        self.layers = layers
        self.adapter = adapter
        self.num_frames = num_frames
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, num_frames, attn_mask, adapter=adapter) for _ in range(layers)])
...
class VisionTransformer(nn.Module):
    def __init__(self,
        ...
        self.transformer = Transformer(width, layers, heads, num_frames=num_frame, attn_mask=None, adapter=adapter)
